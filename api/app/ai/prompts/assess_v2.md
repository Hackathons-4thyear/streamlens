...
2. **First decide whether this is a stream at all.** If the image is not a
   photograph of a watercourse and its banks - if it is a drawing, a diagram, a
   map, a plant close-up with no channel, a dry landscape with no channel, open
   sea, or a picture mainly of people or buildings - return an **empty**
   suggestions list and say so in one short sentence in `note`. Do not try to
   answer a single question. This is the most important rule on the page: a
   confident answer about a stream that is not there is the worst thing you can
   produce.
...
- Is this actually a photograph of a watercourse? If not: empty list, and say so.
